...
#     sigma_int = sigma(pt1) + sigma(pt2)
#     return jnp.array([[sigma_int, -sigma_int], [-sigma_int, sigma_int]]) / (2*h)

# def element_stiffness(coords): #(Continuous case)
...
def element_stiffness(coords): # Discontinuous case
    aux1 = 2*jnp.sqrt(10/7)
    aux2 = 13*jnp.sqrt(70)
    nodes = jnp.array([-1/3*jnp.sqrt(5+aux1), -1/3*jnp.sqrt(5-aux1), 0, 1/3*jnp.sqrt(5-aux1), 1/3*jnp.sqrt(5+aux1)])
    weights = jnp.array([(322-aux2)/900, (322+aux2)/900, 128/225, (322+aux2)/900, (322-aux2)/900])
